...
create_analysis_vars <- function(enrollment_data,
  ...
  if (create_counts) {
    ...
    # Moderate-drought count: years with SPEI in [extreme_threshold, drought_threshold)
    # i.e. the drought_lag variables (moderate-only indicators)
    moderate_lag_cols <- grep("^drought_lag[0-9]+$", names(data), value = TRUE)
    data[[paste0("drought_moderate_count", cumul_window)]] <-
      rowSums(data[, moderate_lag_cols[1:cumul_window]], na.rm = FALSE)
  ...
  if (create_counts) {
    cat("\nModerate drought count summary (past", cumul_window, "years):\n")
    print(table(data[[paste0("drought_moderate_count", cumul_window)]], useNA = "always"))
    cat("\nExtreme drought count summary (past", cumul_window, "years):\n")
    print(table(data[[paste0("drought_extreme_count", cumul_window)]], useNA = "always"))
  }
...
verify_analysis_data <- function(data, enrollment_data) {
  ...
  required_vars <- c(
    ...
    "spei_cumul",
    "drought_moderate_count5", "drought_extreme_count5",
    "drought_consecutive"
  )
  ...
  count_check <- data %>%
    filter(!is.na(drought_moderate_count5)) %>%
    mutate(
      manual_count = drought_lag1 + drought_lag2 + drought_lag3 +
        drought_lag4 + drought_lag5,
      matches = (drought_moderate_count5 == manual_count)
    )

  cat("Moderate count accuracy:", sprintf("%.1f%%\n", 100 * mean(count_check$matches, na.rm = TRUE)))
  ...
  missing_summary <- data %>%
    summarise(
      ...
      pct_missing_count = 100 * mean(is.na(drought_moderate_count5))
    )
