...
# ==============================================================================
# ROBUSTNESS TEST 5: Fully continuous SPEI specification
#
# All prior tests dichotomise SPEI into a binary or categorical drought indicator,
# requiring a threshold choice.  This test avoids any threshold by entering raw
# SPEI directly as a continuous predictor:
#
#   Contemporaneous: spei      (continuous; 1 unit = 1 SD; higher = wetter)
#   Cumulative:      spei_cumul (rolling 5-year sum of lagged SPEI values)
#
# Interpretation: a negative AME on spei means drier conditions (lower SPEI)
# reduce enrollment probability.
# ==============================================================================

library(marginaleffects)

# M1: Average effects (no interactions)
r5_m1 <- feols(enrolled ~ spei + spei_cumul +
                 school_phase + hh_income + gender + race |
                 district + year,
               data = data_school, cluster = ~district)

# M2: All interactions
r5_m2 <- feols(enrolled ~ spei*spei_cumul +
                 spei*school_phase +
                 spei*hh_income +
                 spei*gender +
                 spei*race +
...
etable(r5_m1, r5_m2,
       tex = TRUE, file = "robustness_continuous_spei_results.tex")

# ------------------------------------------------------------------------------
# Average Marginal Effects of continuous SPEI (contemporaneous)
# M1 for overall; M2 for subgroup AMEs (interaction-adjusted)
# ------------------------------------------------------------------------------

ame_r5_overall <- avg_slopes(r5_m1, variables = "spei")

ame_r5_race <- avg_slopes(r5_m2, variables = "spei", by = "race") %>%
  mutate(race = factor(race, levels = c("Black", "Coloured", "White", "Asian/Indian")))

ame_r5_gender <- avg_slopes(r5_m2, variables = "spei", by = "gender")

ame_r5_phase <- avg_slopes(r5_m2, variables = "spei", by = "school_phase") %>%
...
ggplot(ame_r5_race,
  ...
  labs(x = "AME on Enrollment Probability (pp)",
       y = NULL,
       title = "Continuous SPEI: AME by Race",
       subtitle = "Interaction model (M2); 1 unit = 1 SD increase in SPEI (drier → negative)") +
  theme_clean

# --- Plot: AME by gender ---
ggplot(ame_r5_gender,
       aes(x = estimate, xmin = conf.low, xmax = conf.high,
  ...
  labs(x = "AME on Enrollment Probability (pp)",
       y = NULL,
       title = "Continuous SPEI: AME by Gender",
       subtitle = "Interaction model (M2); 1 unit = 1 SD increase in SPEI (drier → negative)") +
  theme_clean

# --- Plot: AME by school phase ---
ggplot(ame_r5_phase,
       aes(x = estimate, xmin = conf.low, xmax = conf.high,
           y = school_phase, color = school_phase)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  geom_pointrange(size = 0.6) +
  ...
  labs(x = "AME on Enrollment Probability (pp)",
       y = NULL,
       title = "Continuous SPEI: AME by School Phase",
       subtitle = "Interaction model (M2); 1 unit = 1 SD increase in SPEI (drier → negative)") +
  theme_clean

# --- Counterfactual prediction: enrollment across SPEI range, by race ---
cf_r5_race <- avg_predictions(
  r5_m2, vcov = FALSE,
  variables = list(spei = seq(-3, 2, by = 0.25)),
  by = c("spei", "race")
) %>%
  mutate(race = factor(race, levels = c("Black", "Coloured", "White", "Asian/Indian")))

ggplot(cf_r5_race, aes(x = spei, y = estimate, color = race)) +
  geom_line(linewidth = 0.9) +
  geom_vline(xintercept = -1, linetype = "dotted", color = "grey40", alpha = 0.7) +
  annotate("text", x = -1.05, y = max(cf_r5_race$estimate, na.rm = TRUE),
           label = "Drought\nthreshold", hjust = 1, size = 3, color = "grey40") +
  scale_color_brewer(palette = "Set2", name = "Race") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_x_continuous(breaks = seq(-3, 2, by = 1)) +
  labs(x = "SPEI (contemporaneous; higher = wetter)",
       y = "Predicted Enrollment Probability",
       title = "Continuous SPEI: Predicted Enrollment by Race",
       subtitle = "Counterfactual predictions (M2); cumulative SPEI held at observed mean") +
  theme_clean
